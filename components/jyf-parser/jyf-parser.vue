...
<template>
	<view>
		...
		<view id="top" :style="showAm+(selectable?';user-select:text;-webkit-user-select:text':'')" :animation="scaleAm"
			...
			<!--#endif-->
			<!--#ifndef H5-->
			<trees :previewImg="previewImg" :nodes="nodes" :lazy-load="lazyLoad" />
...
<script>
	...
	var rpx = uni.$u.sys('getWindowInfo').screenWidth / 750,
		...
		methods: {
			...
			setContent(html, append) {
				...
				if (append)
					...
					html =
						...
						(this.selectable ? '' : ',user-scalable=no') + '"><base href="' + this.domain +
						'"><div id="parser">' + this._handleHtml(html) +
			...
			navigateTo(obj) {
				...
				var Scroll = (selector, component) => {
					uni.createSelectorQuery().in(component ? component : this).select(selector).boundingClientRect()
						.selectViewport()
						.scrollOffset()
						.exec(res => {
							if (!res || !res[0])
