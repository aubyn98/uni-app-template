...
<template>
	<view class="interlayer">
		<block v-for="(n, index) in ns" v-bind:key="index">
			...
			<image v-else-if="n.name=='img'" class="_img" :style="n.attrs.style" :src="n.attrs.src" lazy-load
				:mode="n.mode||'widthFix'" :show-menu-by-longpress="!n.attrs.ignore" :webp="n.webp"
				:data-attrs="n.attrs" :data-auto="n.auto" :data-i="index" data-source="img" @tap="imgtap"
			...
			<block v-else-if="n.name=='video'">
				<view v-if="!loadVideo||n.lazyLoad" :id="n.attrs.id" :class="'_video '+(n.attrs.class||'')"
					:style="n.attrs.style" :data-i="index" @tap="_loadVideo" />
				<video v-else :id="n.attrs.id" :class="n.attrs.class" :style="n.attrs.style"
					:autoplay="n.attrs.autoplay" :controls="n.attrs.controls" :loop="n.attrs.loop"
					:muted="n.attrs.muted" :poster="n.attrs.poster" :src="n.attrs.source[n.i||0]"
					:unit-id="n.attrs['unit-id']" data-source="video" @error="error" @play="play" />
			</block>
			<!--音频-->
			<audio v-else-if="n.name=='audio'" :class="n.attrs.class" :style="n.attrs.style" :author="n.attrs.author"
				...
				:poster="n.attrs.poster" :src="n.attrs.source[n.i||0]" :data-i="index" data-source="audio"
				@error="error" @play="play" />
			<!--链接-->
			...
			<trees v-else :class="(n.attrs.id||'')+' _'+n.name+' '+(n.attrs.class||'')" :style="n.attrs.style"
				:nodes="n.children" :lazyLoad="lazyLoad" />
			<!--#endif-->
		</block>
	</view>
</template>
