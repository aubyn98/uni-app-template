...
class MpHtmlParser {
	...
	setText() {
		...
		while (1) {
			...
			if (text[i + 1] == '#') {
				...
				else if (en != 'lt' && en != 'gt' && en != 'amp' && en != 'ensp' && en != 'emsp' && en != 'quot' &&
					en != 'apos') {
					i && siblings.push({
						type: 'text',
			...
			if (!close) this.remove(node);
				...
				if (parent && attrs.src)
					if (parent.name == 'video' || parent.name == 'audio')
						...
						if (parent.name == 'picture' && !parent.attrs.src && !(attrs.src.indexOf('.webp') && system
								.includes('iOS')) &&
							(!media || (media.includes('px') &&
								(((i = media.indexOf('min-width')) != -1 && (i = media.indexOf(':', i + 8)) != -
										1 && screenWidth > parseInt(
											media.substr(i + 1))) ||
									((i = media.indexOf('max-width')) != -1 && (i = media.indexOf(':', i +
										8)) != -1 && screenWidth < parseInt(
											media.substr(i + 1)))))))
	...
	remove(node) {
		...
		while (1) {
			...
			if (this.getName(this.section()) == name) {
				// 代码块高亮
				if (name == 'pre') {
					this.data = this.data.substr(0, j + 1) + cfg.highlight(this.data.substring(j + 1, this.i - 5),
							node.attrs) +
	...
	matchAttr(node) {
		...
		for (var i = 0, len = styles.length; i < len; i++) {
			...
			if (value.includes('-webkit') || value.includes('-moz') || value.includes('-ms') || value.includes(
				'-o') || value
				.includes(
					'safe'))
				style += `;${key}:${value}`;
		...
		if (node.name == 'img' || node.name == 'picture') {
			...
			if (check('width')) {
				var parent = this.STACK[this.STACK.length - 1];
				if (styleObj.width.includes('%') && parent && (parent.attrs.style || '').includes(
					'display:inline')) {
