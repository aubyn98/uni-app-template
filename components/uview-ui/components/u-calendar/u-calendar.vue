		...
		<view class="u-calendar">
			...
			<view class="u-calendar__content">
				...
				<view class="u-calendar__content__item" :class="{
				 ...
				 :style="{backgroundColor: getColor(index,1)}" v-for="(item, index) in daysArr" :key="index" @tap="dateClick(index)">
					...
					<view class="u-calendar__content__item__tips" :style="{color:activeColor}" v-if="mode== 'range' && startDate==`${year}-${month}-${index+1}` && startDate!=endDate">
						{{startText}}</view>
			...
			<view class="u-calendar__bottom">
				...
				<view class="u-calendar__bottom__btn">
					<u-button :customStyle="btnStyle" :type="btnType" shape="circle" size="default" @click="btnFix(false)">确定</u-button>
				</view>
			</view>
		</view>
	</u-popup>
			...
			defaultVal: {
				handler(val) {
					...
					this.$nextTick(function() {
						[this.year, this.month, this.day] = val.split('-').map(Number)
					})
				},
				immediate: true
			}
